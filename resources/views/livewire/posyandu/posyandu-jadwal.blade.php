            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col" wire:click.stop>
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
                    <h3 class="text-xl font-bold text-gray-800 flex items-center">
                        ...
                        {{ \Carbon\Carbon::parse($selectedDate)->locale('id')->translatedFormat('d F Y') }}
                    </h3>
                    ...
                    <div>
                        ...
                        @if($selectedDateJadwals->isNotEmpty())
                            <div class="space-y-2 mb-4">
                                @foreach($selectedDateJadwals as $j)
                                    @if($j->petugasKesehatan)
                                        @php $pv = strtolower($j->presensi ?? 'belum_hadir'); @endphp
                                        <div class="flex items-center justify-between p-3 rounded-lg border {{ $pv === 'hadir' ? 'bg-green-50 border-green-200' : ($pv === 'tidak_hadir' ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200') }}">
                                            <div class="flex-1">
                                                <span class="font-semibold text-gray-800">{{ $j->petugasKesehatan->nama_petugas_kesehatan }}</span>
                                                <span class="text-xs text-gray-500 ml-2">{{ $j->petugasKesehatan->bidan ?? '-' }}</span>
                                            </div>
                                            <select wire:change="updatePresensi({{ $j->id_jadwal }}, $event.target.value)"
                                                    class="text-xs px-2 py-1 rounded border focus:ring-1 focus:ring-primary focus:border-primary mr-2">
                                                <option value="belum_hadir" {{ $pv === 'belum_hadir' ? 'selected' : '' }}>Belum</option>
                                                <option value="hadir" {{ $pv === 'hadir' ? 'selected' : '' }}>Hadir</option>
                                                <option value="tidak_hadir" {{ $pv === 'tidak_hadir' ? 'selected' : '' }}>Tidak</option>
                                            </select>
                                            <button type="button" wire:click="openEditModal({{ $j->id_jadwal }})" class="text-primary hover:bg-primary/10 rounded p-1" title="Edit">
                ...
                <div class="px-6 py-4 border-t border-gray-200 shrink-0 flex gap-3">
                    ...
                    <button type="button" wire:click="closeModal" class="flex-1 px-5 py-3 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition font-medium">
                        Tutup
                    </button>
                </div>
            </div>
