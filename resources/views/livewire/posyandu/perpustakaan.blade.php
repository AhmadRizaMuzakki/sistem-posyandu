    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-500 mt-1">Kelola koleksi buku {{ $posyandu->nama_posyandu }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-xl font-bold text-gray-800 flex items-center">
                    <i class="ph ph-books text-2xl mr-3 text-primary"></i>
                    Koleksi Buku
                </h2>
            ...
            @if($viewingBook->file_path)
                ...
                <div class="fixed inset-0 z-50 overflow-hidden bg-gray-900/95" aria-modal="true"
                    ...
                    <div x-show="!loading && isMobile" x-cloak class="h-full overflow-y-auto pt-20 pb-4 px-4">
                        <div x-ref="mobileContainer" class="max-w-lg mx-auto space-y-4"></div>
                    </div>
                    ...
                    <div x-show="!loading && !isMobile" x-cloak class="h-full flex items-center justify-center px-4 py-20">
                        <div class="relative w-full max-w-5xl h-[70vh]">
                            ...
                            <button @click="prevPage()" class="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-16 p-4 rounded-full bg-white/10 text-white hover:bg-white/20 transition disabled:opacity-30" :disabled="currentPage <= 1">
                                <i class="ph ph-caret-left text-3xl"></i>
                            </button>
                            <button @click="nextPage()" class="absolute right-0 top-1/2 -translate-y-1/2 translate-x-16 p-4 rounded-full bg-white/10 text-white hover:bg-white/20 transition disabled:opacity-30" :disabled="currentPage >= totalPages">
                        ...
                        <div class="max-w-6xl mx-auto">
                            <div class="flex items-center justify-center gap-4 text-white">
                                ...
                                <div class="flex items-center gap-2">
                                    <button @click="goToPage(1)" class="px-2 py-1 text-sm hover:bg-white/20 rounded"><i class="ph ph-skip-back"></i></button>
                                    <button @click="prevPage()" class="px-2 py-1 text-sm hover:bg-white/20 rounded"><i class="ph ph-caret-left"></i></button>
                                    ...
                                    <button @click="nextPage()" class="px-2 py-1 text-sm hover:bg-white/20 rounded"><i class="ph ph-caret-right"></i></button>
                                    <button @click="goToPage(totalPages)" class="px-2 py-1 text-sm hover:bg-white/20 rounded"><i class="ph ph-skip-forward"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <script>
                    ...
                    function pdfViewer(config) {
                        ...
                        return {
                            pdfUrl: config.pdfUrl, title: config.title, instanceId: instanceId,
                            currentPage: 1, totalPages: 0, loading: true,
                            isMobile: window.innerWidth < 768, mobileRendered: false,
                            
                            async init() { this.checkMobile(); await this.loadPdf(); },
                            checkMobile() {
                                const wasMobile = this.isMobile;
                                this.isMobile = window.innerWidth < 768;
                                if (wasMobile !== this.isMobile && !this.loading) {
                                    this.$nextTick(() => {
                                        if (this.isMobile && !this.mobileRendered) this.renderMobilePages();
                                        else if (!this.isMobile) this.renderDesktopPages();
                                    });
                                }
                            },
                            async loadPdf() {
                                try {
                                    const doc = await pdfjsLib.getDocument(this.pdfUrl).promise;
                                    window._pdfDocuments[this.instanceId] = doc;
                                    this.totalPages = doc.numPages;
                                    this.loading = false;
                                    await this.$nextTick();
                                    setTimeout(() => this.isMobile ? this.renderMobilePages() : this.renderDesktopPages(), 100);
                            ...
                            async renderDesktopPages() {
                                ...
                                if (!left || !right || !doc) return;
                                left.getContext('2d').clearRect(0, 0, left.width, left.height);
                                right.getContext('2d').clearRect(0, 0, right.width, right.height);
                                if (this.currentPage <= this.totalPages) await this.renderPage(this.currentPage, right);
                                if (this.currentPage > 1) await this.renderPage(this.currentPage - 1, left);
                            },
                            ...
                            async nextPage() { if (this.currentPage < this.totalPages) { this.currentPage++; await this.renderDesktopPages(); } },
                            async prevPage() { if (this.currentPage > 1) { this.currentPage--; await this.renderDesktopPages(); } },
                            async goToPage(p) { this.currentPage = Math.max(1, Math.min(p, this.totalPages)); await this.renderDesktopPages(); },
                            destroy() { delete window._pdfDocuments[this.instanceId]; }
                        }
                    }
                </script>
            @else
                                ...
                                <div class="relative w-1/2 h-full bg-white shadow-2xl rounded-l-lg overflow-hidden" :class="currentPage === 0 ? 'opacity-50' : ''">
                                    ...
                                    <template x-if="currentPage === 0">
                                        <div class="w-full h-full bg-gradient-to-r from-gray-200 to-gray-100 flex items-center justify-center">
                                            <div class="text-center text-gray-400"><i class="ph ph-book-open text-6xl mb-4"></i><p class="text-sm">Halaman Depan</p></div>
                                        </div>
                                    </template>
                                    <div class="absolute bottom-4 left-4 px-3 py-1 bg-black/50 text-white text-sm rounded-full" x-show="currentPage > 0" x-text="currentPage"></div>
                                </div>
                                <div class="w-2 h-full bg-gradient-to-r from-gray-400 via-gray-300 to-gray-400 shadow-inner"></div>
                                <div class="relative w-1/2 h-full bg-white shadow-2xl rounded-r-lg overflow-hidden" :class="currentPage >= pages.length ? 'opacity-50' : ''">
                                    ...
                                    <template x-if="currentPage >= pages.length">
                                        <div class="w-full h-full bg-gradient-to-l from-gray-200 to-gray-100 flex items-center justify-center">
                                            <div class="text-center text-gray-400"><i class="ph ph-check-circle text-6xl mb-4"></i><p class="text-sm">Selesai</p></div>
                                        </div>
                                    </template>
                                    <div class="absolute bottom-4 right-4 px-3 py-1 bg-black/50 text-white text-sm rounded-full" x-show="currentPage < pages.length" x-text="currentPage + 1"></div>
                                </div>
                            </div>
                    ...
                    <div x-show="!isMobile" class="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black/80 to-transparent p-4">
                        <div class="max-w-6xl mx-auto">
                            ...
                            <div class="mt-4 flex gap-2 justify-center overflow-x-auto pb-2 max-w-full">
                                <template x-for="(page, index) in pages" :key="index">
                                    <button @click="goToPage(index)" class="flex-shrink-0 w-12 h-16 rounded overflow-hidden border-2 transition-all hover:scale-110"
                ...
                <script>
                    function imageViewer(config) {
                        return {
                            pages: config.pages || [], title: config.title || '', currentPage: 0,
                            isMobile: window.innerWidth < 768,
                            checkMobile() { this.isMobile = window.innerWidth < 768; },
                            nextPage() { if (this.currentPage < this.pages.length) { this.currentPage += 2; if (this.currentPage > this.pages.length) this.currentPage = this.pages.length; } },
                            prevPage() { if (this.currentPage > 0) { this.currentPage -= 2; if (this.currentPage < 0) this.currentPage = 0; } },
                            goToPage(index) { this.currentPage = index % 2 === 0 ? index : index - 1; }
                        }
                    }
                </script>
            @endif
