                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    @foreach($items as $item)
                        <div class="group relative aspect-[4/3] rounded-lg overflow-hidden bg-gray-100 border border-gray-200">
                            ...
                            @if($item->tanggal_foto || $item->caption)
                                <div class="absolute inset-x-0 bottom-0 p-2 bg-black/60 text-white text-xs space-y-0.5">
                                    @if($item->tanggal_foto)
                                        <p class="font-medium">{{ $item->tanggal_foto->format('d/m/Y') }}</p>
                                    @endif
                                    @if($item->caption)
                                        <p class="line-clamp-2 opacity-90">{{ $item->caption }}</p>
                                    @endif
                                </div>
                            @endif
