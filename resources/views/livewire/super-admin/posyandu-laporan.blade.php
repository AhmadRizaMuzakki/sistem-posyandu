<div>
    <div class="space-y-6">
        ...
        {{-- Grup Laporan Absensi Bayi/Balita --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center gap-2 mb-4 pb-3 border-b border-gray-200">
                <i class="ph ph-baby text-2xl text-amber-600"></i>
                <h2 class="text-xl font-semibold text-gray-800">Laporan Absensi Bayi/Balita</h2>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Bulan</label>
                            <select id="filterBulanAbsensiBayi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary">
                                @foreach(range(1, 12) as $m)
                                    <option value="{{ $m }}" {{ $m == now()->month ? 'selected' : '' }}>{{ \Carbon\Carbon::create(now()->year, $m, 1)->locale('id')->translatedFormat('F') }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Tahun</label>
                            <select id="filterTahunAbsensiBayi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary">
                                @foreach(range(now()->year, now()->year - 2) as $y)
                                    <option value="{{ $y }}" {{ $y == now()->year ? 'selected' : '' }}>{{ $y }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <select id="filterStatusAbsensiBayi" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-primary focus:border-primary">
                            <option value="">Semua (Hadir & Tidak Hadir)</option>
                            <option value="hadir">Hadir</option>
                            <option value="tidak_hadir">Tidak Hadir</option>
                        </select>
                    ...
                    <div>
                        <button onclick="exportAbsensiBayiPdf()" class="w-full inline-flex items-center justify-center px-4 py-2 rounded-lg bg-amber-600 text-white text-sm font-medium shadow-sm hover:bg-amber-700 transition-colors">
                            <i class="ph ph-file-pdf text-lg mr-2"></i>
                            Export PDF Laporan Absensi Bayi/Balita
                        </button>
                    </div>
                </div>
            </div>
...
<script>
    function exportAbsensiPdf() {
        ...
        let url = '{{ route("superadmin.posyandu.laporan.absensi.pdf", ["id" => encrypt($posyandu->id_posyandu)]) }}?bulan=' + bulan + '&tahun=' + tahun;
        if (presensi) url += '&presensi=' + presensi;
        window.open(url, '_blank');
    }

    function exportAbsensiBayiPdf() {
        const bulan = document.getElementById('filterBulanAbsensiBayi').value;
        const tahun = document.getElementById('filterTahunAbsensiBayi').value;
        const status = document.getElementById('filterStatusAbsensiBayi').value;
        let url = '{{ route("superadmin.posyandu.laporan.absensi-bayi.pdf", ["id" => encrypt($posyandu->id_posyandu)]) }}?bulan=' + bulan + '&tahun=' + tahun;
