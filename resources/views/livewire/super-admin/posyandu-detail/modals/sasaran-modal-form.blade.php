                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative" x-data="{ 
                                ...
                                selectedId: '',
                                selectedName: '',
                                fieldName: '{{ $prefix }}id_users_sasaran'
                            }" x-init="
                                selectedId = $wire.get('{{ $prefix }}id_users_sasaran') || '';
                                if (selectedId) {
                            ...
                            ">
                                ...
                                <div x-show="open" 
                                     ...
                                     style="display: none;">
                                    <ul class="py-1">
                                        <li @click="selectedId = ''; searchText = ''; open = false; $wire.set(fieldName, '')" 
                                            class="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 cursor-pointer">
                                            -- Pilih User --
                                        </li>
                                        @if($users->count() > 0)
                                            @foreach($users as $user)
                                                <li @click="selectedId = '{{ $user->id }}'; selectedName = '{{ $user->name }} ({{ $user->email }})'; searchText = selectedName; open = false; $wire.set(fieldName, '{{ $user->id }}')" 
