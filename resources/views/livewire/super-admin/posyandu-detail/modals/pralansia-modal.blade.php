                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" inputmode="numeric" maxlength="16" pattern="[0-9]*" wire:model="nik_sasaran_pralansia" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="NIK Sasaran">
                        ...
                        <div>
                            ...
                            <div class="relative" x-data="{
                                open: false,
                                searchText: '',
                                selectedNoKk: @entangle('no_kk_sasaran_pralansia'),
                                noKkList: @js($this->getNoKkListPralansia())
                            }" x-init="
                                $watch('selectedNoKk', value => {
                                    if (value) {
                                        ...
                                        if (item) {
                                            ...
                                            setTimeout(() => {
                                                $wire.call('loadDataByNoKkPralansia', value);
                                            }, 150);
                                        } else {
                                            searchText = value;
                                        }
                            ...
                            ">
                                <input
                                    ...
                                    @input="
                                        ...
                                        if (match && !noKkList.find(item => item.no_kk == match[0])) {
                                            ...
                                            $wire.set('no_kk_sasaran_pralansia', match[0]);
                                        }
                                    "
                                ...
                                <div x-show="open"
                                     ...
                                     style="display: none;">
                                    <ul class="py-1">
                                        <li @click="selectedNoKk = ''; searchText = ''; open = false; $wire.set('no_kk_sasaran_pralansia', '')"
                                            class="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 cursor-pointer">
                                            -- Pilih No. KK --
                                        </li>
                                        @if(count($this->getNoKkListPralansia()) > 0)
                                            @foreach($this->getNoKkListPralansia() as $item)
                                                <li @click="
                                                    const noKk = '{{ $item['no_kk'] }}';
                                                    selectedNoKk = noKk;
                                                    searchText = '{{ $item['no_kk'] }} ({{ $item['nama_orangtua'] }}) - {{ $item['jumlah_anggota'] }} anggota';
                                                    open = false;
                                                    $wire.set('no_kk_sasaran_pralansia', noKk);
                                                    setTimeout(() => {
                                                        $wire.call('loadDataByNoKkPralansia', noKk);
                                                    }, 200);
                                                "
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <div>
                                    <select wire:model="hari_lahir_pralansia" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                        <option value="">Hari</option>
                                        @for($i = 1; $i <= 31; $i++)
                                            <option value="{{ $i }}">{{ $i }}</option>
                                        @endfor
                                    </select>
                                    @error('hari_lahir_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <select wire:model="bulan_lahir_pralansia" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                    ...
                                    @error('bulan_lahir_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <input type="number" wire:model="tahun_lahir_pralansia" min="1900" max="{{ date('Y') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Tahun">
                                    @error('tahun_lahir_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <select wire:model="status_keluarga_pralansia" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                <option value="">Pilih Status Keluarga...</option>
                                <option value="kepala keluarga">Kepala Keluarga</option>
                                <option value="istri">Istri</option>
                                <option value="anak">Anak</option>
                            </select>
                            @error('status_keluarga_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <select wire:model="pendidikan_pralansia" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                <x-pendidikan-options />
                            </select>
                            @error('pendidikan_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div class="md:col-span-2">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="rt_pralansia" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="RT">
                            @error('rt_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">RW</label>
                            <input type="text" wire:model="rw_pralansia" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="RW">
                            @error('rw_pralansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
