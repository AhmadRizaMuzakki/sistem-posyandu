                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <div>
                                    <select wire:model="hari_lahir_lansia" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                        <option value="">Hari</option>
                                        @for($i = 1; $i <= 31; $i++)
                                            <option value="{{ $i }}">{{ $i }}</option>
                                        @endfor
                                    </select>
                                    @error('hari_lahir_lansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <select wire:model="bulan_lahir_lansia" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                    ...
                                    @error('bulan_lahir_lansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <input type="number" wire:model="tahun_lahir_lansia" min="1900" max="{{ date('Y') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Tahun">
                                    @error('tahun_lahir_lansia') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="pekerjaan_lansia" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Masukkan pekerjaan">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative" x-data="{
                                open: false,
                                searchText: '',
                                selectedId: @entangle('id_users_sasaran_lansia'),
                                selectedName: ''
                            }" x-init="
                            ...
                            ">
                                ...
                                <div x-show="open"
                                     ...
                                     style="display: none;">
                                    <ul class="py-1">
                                        <li @click="selectedId = ''; searchText = ''; open = false; $wire.set('id_users_sasaran_lansia', '')"
                                            class="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 cursor-pointer">
                                            -- Pilih User --
                                        </li>
                                        @if($users->count() > 0)
                                            @foreach($users as $user)
                                                <li @click="selectedId = '{{ $user->id }}'; selectedName = '{{ $user->name }} ({{ $user->email }})'; searchText = selectedName; open = false; $wire.set('id_users_sasaran_lansia', '{{ $user->id }}')"
