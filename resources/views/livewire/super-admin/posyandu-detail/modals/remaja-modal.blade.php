                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" inputmode="numeric" maxlength="16" pattern="[0-9]*" wire:model="nik_sasaran_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="NIK Sasaran">
                        ...
                        <div>
                            ...
                            <div class="relative" x-data="{
                                open: false,
                                searchText: '',
                                selectedNoKk: @entangle('no_kk_sasaran_remaja'),
                                noKkList: @js($this->getNoKkListRemaja())
                            }" x-init="
                                $watch('selectedNoKk', value => {
                                    if (value) {
                                        ...
                                        if (item) {
                                            ...
                                            // Auto-fill data orangtua ketika No KK dipilih
                                            setTimeout(() => {
                                                $wire.call('loadOrangtuaByNoKkRemaja', value);
                                            }, 150);
                                        } else {
                                            searchText = value;
                                        }
                            ...
                            ">
                                <input
                                    ...
                                    @input="
                                        ...
                                        if (match && !noKkList.find(item => item.no_kk == match[0])) {
                                            ...
                                            $wire.set('no_kk_sasaran_remaja', match[0]);
                                        }
                                    "
                                ...
                                <div x-show="open"
                                     ...
                                     style="display: none;">
                                    <ul class="py-1">
                                        <li @click="selectedNoKk = ''; searchText = ''; open = false; $wire.set('no_kk_sasaran_remaja', '')"
                                            class="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 cursor-pointer">
                                            -- Pilih No. KK --
                                        </li>
                                        @if(count($this->getNoKkListRemaja()) > 0)
                                            @foreach($this->getNoKkListRemaja() as $item)
                                                <li @click="
                                                    const noKk = '{{ $item['no_kk'] }}';
                                                    selectedNoKk = noKk;
                                                    searchText = '{{ $item['no_kk'] }} ({{ $item['nama_orangtua'] }}) - {{ $item['jumlah_anggota'] }} anggota';
                                                    open = false;
                                                    $wire.set('no_kk_sasaran_remaja', noKk);
                                                    // Panggil method untuk auto-fill data orangtua
                                                    setTimeout(() => {
                                                        $wire.call('loadOrangtuaByNoKkRemaja', noKk);
                                                    }, 200);
                                                "
                                                    x-show="!searchText || '{{ strtolower($item['no_kk'] . ' ' . $item['nama_orangtua']) }}'.includes(searchText.toLowerCase())"
                                                    class="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer"
                                                    :class="selectedNoKk == '{{ $item['no_kk'] }}' ? 'bg-blue-50 font-medium' : ''">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <div>
                                    <select wire:model="hari_lahir_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                        <option value="">Hari</option>
                                        @for($i = 1; $i <= 31; $i++)
                                            <option value="{{ $i }}">{{ $i }}</option>
                                        @endfor
                                    </select>
                                    @error('hari_lahir_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <select wire:model="bulan_lahir_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                    ...
                                    @error('bulan_lahir_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <input type="number" wire:model="tahun_lahir_remaja" min="1900" max="{{ date('Y') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Tahun">
                                    @error('tahun_lahir_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                            </div>
                        ...
                        <div>
                            ...
                            <select wire:model="status_keluarga_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                <option value="">Pilih Status Keluarga...</option>
                                <option value="kepala keluarga">Kepala Keluarga</option>
                                <option value="istri">Istri</option>
                                <option value="anak">Anak</option>
                            </select>
                            @error('status_keluarga_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Umur (tahun)</label>
                            <input type="number" wire:model="umur_sasaran_remaja" readonly class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary bg-gray-100 cursor-not-allowed">
                        ...
                        <div>
                            ...
                            @error('pendidikan_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div class="md:col-span-2">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="rt_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="RT">
                            @error('rt_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">RW</label>
                            <input type="text" wire:model="rw_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="RW">
                            @error('rw_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" inputmode="numeric" maxlength="16" pattern="[0-9]*" wire:model="nik_orangtua_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="NIK salah satu orangtua">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="nama_orangtua_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Nama lengkap orangtua">
                            @error('nama_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Tempat Lahir Orangtua <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="tempat_lahir_orangtua_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Contoh: Bandung">
                            @error('tempat_lahir_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Tanggal Lahir Orangtua <span class="text-red-500">*</span></label>
                            <div class="grid grid-cols-3 gap-2">
                                <div>
                                    <select wire:model="hari_lahir_orangtua_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                        <option value="">Hari</option>
                                        @for($i = 1; $i <= 31; $i++)
                                            <option value="{{ $i }}">{{ $i }}</option>
                                        @endfor
                                    </select>
                                    @error('hari_lahir_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <select wire:model="bulan_lahir_orangtua_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                        <option value="">Bulan</option>
                                        <option value="1">Januari</option>
                                        <option value="2">Februari</option>
                                        <option value="3">Maret</option>
                                    ...
                                    @error('bulan_lahir_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                                <div>
                                    <input type="number" wire:model="tahun_lahir_orangtua_remaja" min="1900" max="{{ date('Y') }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Tahun">
                                    @error('tahun_lahir_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                                </div>
                            </div>
                            @error('tanggal_lahir_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Jenis Kelamin Orangtua <span class="text-red-500">*</span></label>
                            <select wire:model="kelamin_orangtua_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                <option value="">Pilih Jenis Kelamin...</option>
                                <option value="Laki-laki">Laki-laki</option>
                                <option value="Perempuan">Perempuan</option>
                            </select>
                        ...
                        <div>
                            ...
                            @error('pekerjaan_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            ...
                            @error('pendidikan_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Kepersertaan BPJS Orangtua</label>
                            <select wire:model="kepersertaan_bpjs_orangtua_remaja" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary">
                                <option value="">Pilih...</option>
                                <option value="PBI">PBI</option>
                                <option value="NON PBI">NON PBI</option>
                            </select>
                            @error('kepersertaan_bpjs_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Nomor BPJS Orangtua</label>
                            <input type="text" wire:model="nomor_bpjs_orangtua_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Nomor kartu BPJS orangtua">
                            @error('nomor_bpjs_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                        <div>
                            <label class="block text-gray-700 text-sm font-bold mb-2">Nomor Telepon Orangtua</label>
                            <input type="text" wire:model="nomor_telepon_orangtua_remaja" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-primary focus:border-primary" placeholder="Nomor WA/Telp orangtua">
                            @error('nomor_telepon_orangtua_remaja') <span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                        </div>
                    </div>
                </div>
