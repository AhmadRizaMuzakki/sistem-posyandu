...
<div class="p-6">
    ...
    @if($todayJadwals->isEmpty())
        <div class="bg-gradient-to-br from-yellow-50 to-orange-50 border-2 border-yellow-200 rounded-2xl p-12 text-center shadow-sm">
            ...
            <p class="text-gray-600 max-w-md mx-auto">Belum ada jadwal petugas untuk tanggal hari ini. Silakan tambahkan jadwal terlebih dahulu di tab Jadwal.</p>
        </div>
    @else
        @php
            $totalJadwal = $todayJadwals->count();
            $hadirCount = $todayJadwals->where('presensi', 'hadir')->count();
            $tidakHadirCount = $todayJadwals->where('presensi', 'tidak_hadir')->count();
            $belumHadirCount = $todayJadwals->where('presensi', 'belum_hadir')->count();
        @endphp
