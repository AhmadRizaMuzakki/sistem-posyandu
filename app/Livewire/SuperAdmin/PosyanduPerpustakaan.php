...
class PosyanduPerpustakaan extends Component
{
    ...
    {
        $book = Perpustakaan::where('id_posyandu', $this->posyanduId)->findOrFail($id);
    ...
    {
        $this->viewingBook = Perpustakaan::where('id_posyandu', $this->posyanduId)->findOrFail($id);
    ...
    public function updateBook()
    {
        ...
        $book = Perpustakaan::where('id_posyandu', $this->posyanduId)->findOrFail($this->editingId);
        $book->update([
            'judul' => $this->judul,
            'deskripsi' => $this->deskripsi,
            'penulis' => $this->penulis,
    ...
    {
        $items = Perpustakaan::where('id_posyandu', $this->posyanduId)
            ->latest()
            ->paginate(12);

        return view('livewire.super-admin.posyandu-perpustakaan', [
            'items' => $items,
